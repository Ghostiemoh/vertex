...
import { Download, User, Briefcase, Loader2, Building, ShieldCheck, AlertCircle, Clock, Scale, CreditCard, Trash2, Plus, Calendar as CalendarIcon } from "lucide-react";
...
export default function ContractPage() {
  ...
  useEffect(() => {
    if (effectiveWallet) {
      QRCode.toDataURL(`https://vertex.vercel.app/verify/${contractId}`, { margin: 1, width: 200, color: { dark: "#000000", light: "#ffffff" } })
        .then(setPreviewQr)
        .catch(() => setPreviewQr(""));
    }
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const qrDataUrl = await QRCode.toDataURL(`https://vertex.vercel.app/verify/${contractId}`, { margin: 1, width: 300 });
                    ...
                    {previewQr && (
                      <div className="flex-shrink-0 flex flex-col items-center gap-1 -mt-2">
                        <div className="w-20 h-20 border border-zinc-200 rounded-lg overflow-hidden bg-white p-1">
                          <img src={previewQr} alt="Verification QR" className="w-full h-full object-contain" />
                        </div>
